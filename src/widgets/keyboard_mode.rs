        ...
        popover.connect_show(move |_| {
            *active_popovers_show.borrow_mut() += 1;
            if let Some(window) = window_weak_show.upgrade() {
                ...
                info!("{} popover shown - keyboard mode set to OnDemand (active popovers: {})", 
                      widget_name_show, *active_popovers_show.borrow());
            }
        });
            ...
            *active_popovers_hide.borrow_mut() -= 1;
            ...
            if count == 0 {
                if let Some(window) = window_weak_hide.upgrade() {
                    ...
                    info!("{} popover hidden - keyboard mode set to None", widget_name_hide);
                }
            } else {
                info!("{} popover hidden - keeping keyboard mode (active popovers: {})", 
