...
use gtk4::prelude::*;
use gtk4::{
    ApplicationWindow, Box, Button, Image, Label, Orientation, Popover, Separator,
};
...
impl Places {
    pub fn new(
        ...
        popover.connect_show(move |_| {
            *active_popovers_show.borrow_mut() += 1;
            if let Some(window) = window_weak_show.upgrade() {
                ...
                info!(
                    "Places popover shown - keyboard mode set to OnDemand (active popovers: {})",
        ...
        popover.connect_hide(move |_| {
            *active_popovers_hide.borrow_mut() -= 1;
            ...
            if count == 0 {
                ...
                info!(
                    "Places popover hidden - keeping keyboard mode (active popovers: {})",
                    count
                );
            }
        });
        ...
        places_content.set_margin_top(5);
        places_content.set_margin_bottom(5);
        places_content.set_size_request(250, -1);

        popover.set_child(Some(&places_content));
