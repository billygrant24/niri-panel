...
use gtk4::prelude::*;
use gtk4::{
    ApplicationWindow, Box, Button, Calendar, CheckButton, Entry, Label, ListBox, ListBoxRow,
...
impl Clock {
    pub fn new(
        ...
        popover.connect_show(move |_| {
            *active_popovers_show.borrow_mut() += 1;
            if let Some(window) = window_weak_show.upgrade() {
                ...
                info!(
                    "Clock popover shown - keyboard mode set to OnDemand (active popovers: {})",
        ...
        popover.connect_hide(move |_| {
            *active_popovers_hide.borrow_mut() -= 1;
            ...
            if count == 0 {
                ...
                info!(
                    "Clock popover hidden - keeping keyboard mode (active popovers: {})",
                    count
                );
            }
        });
        ...
        calendar.connect_day_selected(move |cal| {
            let date = format!("{}-{:02}-{:02}", cal.year(), cal.month() + 1, cal.day());
            ...
            if let (
                ...
                active_list_weak.upgrade(),
                completed_list_weak.upgrade(),
                date_label_weak.upgrade(),
                todo_count_weak.upgrade(),
        ...
        let add_todo = move || {
            if let (
                ...
                calendar_for_add.upgrade(),
                active_list_for_add.upgrade(),
                completed_list_for_add.upgrade(),
                todo_entry_weak.upgrade(),
                todo_count_for_add.upgrade(),
                completed_count_for_add.upgrade(),
            ) {
                let text = entry.text();
                if !text.is_empty() {
                    let date = format!("{}-{:02}-{:02}", cal.year(), cal.month() + 1, cal.day());
        ...
        button.connect_clicked(move |_| {
            ...
            if let (
                ...
                calendar_for_show.upgrade(),
                active_list_for_show.upgrade(),
                completed_list_for_show.upgrade(),
                date_label_for_show.upgrade(),
                todo_count_for_show.upgrade(),
                completed_count_for_show.upgrade(),
            ) {
                let now = Local::now();
        ...
        if let Some(due_date) = &todo.due_date {
            if due_date != current_date && !in_completed_section {
                ...
                if let (Ok(due), Ok(today_date)) = (
                    ...
                    if due < today_date {
                        due_label.set_markup(&format!(
                            "<span color='#f27835'>Overdue: {}</span>",
                                            ...
                                            widget.downcast_ref::<ScrolledWindow>()
                                        {
                                            if let Some(list_child) = scroll.child() {
                                                if let Some(list_box) =
                                                    list_child.downcast_ref::<ListBox>()
                                                {
        ...
        match Self::todo_store_path() {
            Ok(path) => {
                ...
                match fs::read_to_string(&path) {
                    Ok(content) => match serde_json::from_str::<TodoStore>(&content) {
                        Ok(store) => {
