...
use gtk4::prelude::*;
use gtk4::{Button, Image, Popover, Box, Orientation, Label, Entry, ListBox, ListBoxRow, ScrolledWindow, ApplicationWindow, SearchEntry, Separator, Stack, StackSwitcher};
...
impl Secrets {
    ...
    fn populate_lists(passwords_list: &ListBox, otp_list: &ListBox, entries: &[SecretEntry], query: &str) {
    ...
    fn create_secret_row(entry: &SecretEntry, is_otp: bool) -> ListBoxRow {
        ...
        if is_otp {
            ...
            refresh_button.connect_clicked(move |button| {
                Self::generate_otp(&entry_path, button, otp_label_weak.clone(), timer_label_weak.clone());
            });
            ...
            .output()
        {
            Ok(output) => {
                if output.status.success() {
                    ...
                    error!("Failed to copy password: {}", String::from_utf8_lossy(&output.stderr));
                }
            }
            Err(e) => {
    ...
    fn generate_otp(path: &str, button: &Button, otp_label_weak: WeakRef<Label>, timer_label_weak: WeakRef<Label>) {
        match Command::new("pass")
            .arg("otp")
            .arg(path)
            .output()
        {
            Ok(output) => {
                if output.status.success() {
                    ...
                    error!("Failed to generate OTP: {}", String::from_utf8_lossy(&output.stderr));
                }
            }
            Err(e) => {
                ...
                if output.status.success() {
                    ...
                    for line in lines.iter().skip(1) {
                        if line.starts_with("Username:") || line.starts_with("username:") {
                            info.username = Some(line.split(':').nth(1).unwrap_or("").trim().to_string());
                        } else if line.starts_with("URL:") || line.starts_with("url:") {
                            info.url = Some(line.split(':').nth(1).unwrap_or("").trim().to_string());
            ...
            .output()
        {
            Ok(output) => {
                if output.status.success() {
                    ...
                    warn!("Failed to sync: {}", String::from_utf8_lossy(&output.stderr));
                }
            }
            Err(e) => {
