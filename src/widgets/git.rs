use crate::config::{GitRepository, GitService, PanelConfig};
use anyhow::Result;
use gtk4::glib::{clone, WeakRef};
use gtk4::prelude::*;
use gtk4::{
...
impl Git {
    pub fn new(
        ...
        popover.connect_show(move |_| {
            *active_popovers_show.borrow_mut() += 1;
            if let Some(window) = window_weak_show.upgrade() {
                ...
                info!(
                    "Git popover shown - keyboard mode set to OnDemand (active popovers: {})",
        ...
        popover.connect_hide(move |_| {
            *active_popovers_hide.borrow_mut() -= 1;
            ...
            if count == 0 {
                ...
                info!(
                    "Git popover hidden - keeping keyboard mode (active popovers: {})",
                    count
                );
            }
        });
        ...
        search_entry.connect_search_changed(move |entry| {
            ...
            let search_term = if search_text.is_empty() {
                None
            } else {
                Some(search_text)
            };
            Self::populate_repo_list(
                &repos_list_for_search,
                &repositories_for_search.borrow(),
                search_term,
                None,
            );
        });
        ...
        service_combo.connect_selected_notify(move |combo| {
            ...
            Self::populate_repo_list(
                &repos_list_for_filter,
                &repositories_for_filter.borrow(),
                search_term,
                service_filter,
            );
        });
    ...
    fn populate_repo_list(
        ...
        let filtered_repos: Vec<&GitRepository> = repositories
            .iter()
            .filter(|repo| {
    ...
    fn open_folder(path: &Path) {
        ...
        let file_managers = vec!["nautilus", "nemo", "thunar", "dolphin", "pcmanfm", "caja"];
