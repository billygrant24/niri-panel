...
use gtk4::prelude::*;
use gtk4::{
    ApplicationWindow, Box, Button, Image, Label, ListBox, ListBoxRow, Orientation, Popover,
...
impl Launcher {
        ...
        popover.connect_show(move |_| {
            *active_popovers_show.borrow_mut() += 1;
            if let Some(window) = window_weak_show.upgrade() {
                ...
                info!(
                    "Launcher popover shown - keyboard mode set to OnDemand (active popovers: {})",
        ...
        popover.connect_hide(move |_| {
            *active_popovers_hide.borrow_mut() -= 1;
            ...
            if count == 0 {
                ...
                info!(
                    "Launcher popover hidden - keeping keyboard mode (active popovers: {})",
                    count
                );
            }
        });
    ...
    fn launch_app(exec: &str) {
        ...
        if let Err(e) = Command::new("sh").arg("-c").arg(&clean_exec).spawn() {
